...
source_path = "/data4/liandong/datasets/Librilight/medium"
target_sampling_rate = 16000
target_len = 8.0
target_path = "/data4/liandong/datasets/Librilight/{}k_medium".format(target_sampling_rate//1000)
if not os.path.exists(target_path):
   os.makedirs(target_path)

source_list = glob(f"{source_path}/medium/*/*/*.flac")

def process_file(cur_filepath):
   filename = os.path.split(cur_filepath)[-1].split('.')[0]
   par_rela_path = '/'.join(cur_filepath.split('/')[-4:-1])
